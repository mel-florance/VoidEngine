...
public:
	...
	static inline size_t splitStr(const std::string& txt, std::vector<std::string>& parts, char delimiter)
	{
		...
		{
			parts.push_back(txt.substr(start, pos - start));
			start = pos + 1;

			pos = txt.find(delimiter, start);
		}

		parts.push_back(txt.substr(start, std::min(pos, txt.size()) - start + 1));

		return parts.size();
	}

	static inline float mapValues(float rangeA[], float rangeB[], float value)
	{
